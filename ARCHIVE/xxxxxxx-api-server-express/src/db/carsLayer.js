...
function carsLayer(db) {
  db.serialize(function () {
    db.run(
      "CREATE TABLE if not exists cars (id INTEGER PRIMARY KEY AUTOINCREMENT, year NUMBER, model TEXT, color TEXT)"
    );
  });
  ...
  function createCar(year, model, color, callback) {
    db.serialize(() => {
      ...
      let newId = -1;
      db.each(
        "select * from sqlite_sequence where name='cars'; ",
        (err, row) => {
          newId = row.seq;
        },
        (err, rows) => {
  ...
  function removeCar(id, callback) {
    console.log("  in removeCar of ", id);
    db.serialize(() => {
      getCar(id, (car) => {
